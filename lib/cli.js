...
if ( program.start ) {
  ...
  else if ( fs.existsSync(process.cwd() + '/rs-reporter-config.json') ) {
    server.start(process.cwd() + '/rs-reporter-config.json');
  }
  else {
    console.log('No config file found, please specify');
  }
}

function createNew() {

  var name = typeof program.new === 'boolean' ? 'rs-reporter' : program.new;
  ...
  rl.question('Are you sure you would like to create a new rs-reporter?\n\nProject Name: '+chalk.cyan(name)+'\nProjected will be created at: '+chalk.cyan(process.cwd()+'/'+name)+'\n\nContinue? (y/n)', function(answer) {
    ...
    // Time to run create
      ...
      // Create config file
      var configFile = name + '/rs-reporter-config.json';
      ...
      console.log('Created Config File: %s', configFile);

      // Create empty report file
      var reportFile = name + '/report.json';
      fs.writeFileSync(reportFile, JSON.stringify({}));
      console.log('Created Report File: %s', reportFile);
      process.exit(0);
    }
  });
